...
## Fix — Log-transform V_rf and f_rf in ElementEncoder (2026-04-25)

V_rf (0.01–20 MV, 3 orders of magnitude) and f_rf (0.1–3 GHz, 1.5 orders) are now passed through `log1p` inside `ElementEncoder.forward()` before the normalisation division. The norm scales for those two channels are updated to match the log-transformed range. `log1p` is used instead of `log` so that non-RF elements (V_rf = f_rf = 0) correctly map to 0. No changes to data files or other model components.
